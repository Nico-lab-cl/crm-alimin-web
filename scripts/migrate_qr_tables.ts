...
async function migrate() {
  ...
  let sqlContent = '';
  try {
    const sqlPath = path.join(process.cwd(), 'scripts', 'setup_qr_tables.sql');
    sqlContent = fs.readFileSync(sqlPath, 'utf8');
  } catch (err) {
    console.error('Error reading setup_qr_tables.sql:', err);
    process.exit(1);
  }
  ...
  for (const conn of connections) {
    ...
    try {
      ...
      console.log(`Connected successfully to ${conn.name}! Running QR schema migration...`);

      // Run migration SQL queries
      await client.query(sqlContent);

      console.log("SUCCESS: 'qr_codes' and 'qr_scans' tables and indexes are updated and ready.");
